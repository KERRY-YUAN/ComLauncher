...
class SettingsTab:
    ...
    def __init__(self, parent_frame, app_instance):
        ...
        # VRAM Mode
        ttk.Label(perf_group, text="显存优化:", anchor=tk.W).grid(row=perf_row, column=0, sticky=tk.W, pady=widget_pady, padx=widget_padx)
        vram_modes = ["全负载(10GB以上)", "高负载(8GB以上)", "中负载(4GB以上)", "低负载(2GB以上)"]
        vram_mode_combo = ttk.Combobox(perf_group, textvariable=self.app.vram_mode_var, values=vram_modes, state="readonly")
        vram_mode_combo.grid(row=perf_row, column=1, sticky="ew", pady=widget_pady, padx=widget_padx)
        perf_row += 1

        # CKPT Precision
        ttk.Label(perf_group, text="CKPT模型精度:", anchor=tk.W).grid(row=perf_row, column=0, sticky=tk.W, pady=widget_pady, padx=widget_padx)
        ckpt_precisions = ["全精度(FP32)", "半精度(FP16)"]
        ckpt_precision_combo = ttk.Combobox(perf_group, textvariable=self.app.ckpt_precision_var, values=ckpt_precisions, state="readonly")
        ckpt_precision_combo.grid(row=perf_row, column=1, sticky="ew", pady=widget_pady, padx=widget_padx)
        perf_row += 1

        # VAE Precision
        ttk.Label(perf_group, text="VAE模型精度:", anchor=tk.W).grid(row=perf_row, column=0, sticky=tk.W, pady=widget_pady, padx=widget_padx)
        ...
        clip_precision_combo = ttk.Combobox(perf_group, textvariable=self.app.clip_precision_var, values=clip_precisions, state="readonly")
        clip_precision_combo.grid(row=perf_row, column=1, sticky="ew", pady=widget_pady, padx=widget_padx)
        perf_row += 1

        # UNET Precision
        ttk.Label(perf_group, text="UNET模型精度:", anchor=tk.W).grid(row=perf_row, column=0, sticky=tk.W, pady=widget_pady, padx=widget_padx)
        unet_precisions = ["半精度(BF16)", "半精度(FP16)", "FP8 (E4M3FN)", "FP8 (E5M2)"]
        unet_precision_combo = ttk.Combobox(perf_group, textvariable=self.app.unet_precision_var, values=unet_precisions, state="readonly")
        unet_precision_combo.grid(row=perf_row, column=1, sticky="ew", pady=widget_pady, padx=widget_padx)
        perf_row += 1

        # CUDA Malloc
        ttk.Label(perf_group, text="CUDA智能内存分配:", anchor=tk.W).grid(row=perf_row, column=0, sticky=tk.W, pady=widget_pady, padx=widget_padx)
        cuda_malloc_options = ["启用", "禁用"]
        cuda_malloc_combo = ttk.Combobox(perf_group, textvariable=self.app.cuda_malloc_var, values=cuda_malloc_options, state="readonly")
        cuda_malloc_combo.grid(row=perf_row, column=1, sticky="ew", pady=widget_pady, padx=widget_padx)
        perf_row += 1

        # IPEX Optimization
        ttk.Label(perf_group, text="IPEX优化:", anchor=tk.W).grid(row=perf_row, column=0, sticky=tk.W, pady=widget_pady, padx=widget_padx)
        ipex_options = ["启用", "禁用"]
        ipex_combo = ttk.Combobox(perf_group, textvariable=self.app.ipex_optimization_var, values=ipex_options, state="readonly")
        ipex_combo.grid(row=perf_row, column=1, sticky="ew", pady=widget_pady, padx=widget_padx)
        perf_row += 1

        # xformers Acceleration
        ttk.Label(perf_group, text="xformers加速:", anchor=tk.W).grid(row=perf_row, column=0, sticky=tk.W, pady=widget_pady, padx=widget_padx)
        xformers_options = ["启用", "禁用"]
        xformers_combo = ttk.Combobox(perf_group, textvariable=self.app.xformers_acceleration_var, values=xformers_options, state="readonly")
